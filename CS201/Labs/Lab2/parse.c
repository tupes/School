...
void convert(FILE* input, FILE* executable)
{
  ...
  double x1_read, y1_read, x2_read, y2_read;
  long x1_sent, y1_sent, x2_sent, y2_sent; 	
  ...
  while (fgets(line, LINE_LEN + 1, input) != NULL)
  {
	...
	/* My code begins here */
    else if (sscanf(line, "%s%lf%lf%lf%lf", command, &x1_read, &y1_read, 
	    &x2_read, &y2_read) == 5 && strncmp(line, "DRAWSegment", 11) == 0)
    {
	x1_sent = lround(x1_read);
	y1_sent = lround(y1_read);
	x2_sent = lround(x2_read);
	y2_sent = lround(y2_read);    
	fprintf(executable, "drawSegment %ld %ld %ld %ld\n", 
	    x1_sent, y1_sent, x2_sent, y2_sent);
    }	    
